# **********************************************************************************************************************
# This file implements the classical Successive Cancellation (SC) and Successive Cancellation List (SCL) algorithms
# that are used to decode polar codes.
#
# This is part of the code for the paper "Decoding Polar Codes using Quantum Approximate Optimization
# Algorithm (QAOA)" as submitted to the "QEC-22" conference (https://qce.quantum.ieee.org/2022/)
#
...
# This is based on 3GPP TS 38.212 Table 5.3.1.2-1: Polar sequence
# Use the "PrintReliabilitySequence.py" file to print this.
...
# **********************************************************************************************************************
def getReliabilitySequence(n):
    return reliabilitySeq[reliabilitySeq<n]
    
# **********************************************************************************************************************
# scDecode: The recursive successive cancellation algorithm:
def scDecode(r, frozenIdx, idx=0):
    n = len(r)
    if n==1:
        # We are at leaf level:
        if idx in frozenIdx: u = 0
        elif r[0]>=0:        u = 0
        else:                u = 1
        return np.int32([u]), np.int32([u])
    
    r2 = r.reshape((2,-1)) # r broken down to 2 rows: 1st half and 2nd half
    
    # Left: (SPC)
    minAbs = np.abs(r2).min(axis=0)
    sign = np.sign(r2).prod(axis=0)
    leftR = sign*minAbs
    ul,ulx = scDecode(leftR, frozenIdx, idx)

    # Right: (Repetition)
    rightR = (np.vstack(((1-2*ulx),[1]*(n//2)))*r2).sum(axis=0)
    ur,urx = scDecode(rightR, frozenIdx, idx+n//2)

    # The first output is used to reconstruct the message
    # The second output is used internally and eventually will provide the codeword
    return np.concatenate((ul,ur)), np.concatenate((ulx^urx,urx))

# **********************************************************************************************************************
# sclDecode: The recursive successive cancellation list algorithm:
# l is the number of top solution candidates to keep.
# The scDecode above is the special case when l=1
# Returns 3 lists
def sclDecode(r, frozenIdx, l=1, idx=0, minsum=True):
    n = len(r)
    if n==1:
        # We are at leaf level:
        if idx in frozenIdx: bits, decisionMetrics = [[0]], [(0 if r[0]>=0 else -r[0])]
        elif r[0]>=0:        bits, decisionMetrics = [[0],[1]], [0, r[0]]
        else:                bits, decisionMetrics = [[1],[0]], [0, -r[0]]
        return np.int8(bits[:l]), np.int8(bits[:l]), np.float32(decisionMetrics[:l])
    ...
    # SPC extrinsic likelihood function
    def f(x): return np.abs(np.log(np.tanh(np.abs(x/2.0))))

    if minsum: lext = np.abs(r2).min(axis=0)  # Minsum approximation (Fast)
    else:      lext = f(f(r2).sum(axis=0))    # The extrinsic likelihood (Slow, precise)
    leftR = sign*lext
    uls, ulxs, pmls = sclDecode(leftR, frozenIdx, l, idx, minsum)

    u = []
    ux = []
    pm = []
    # Right: (Repetition)
    for ul, ulx, pml in zip(uls, ulxs, pmls):
        rightR = (np.vstack(((1-2.0*ulx),[1]*(n//2)))*r2).sum(axis=0)
        urs, urxs, pmrs = sclDecode(rightR, frozenIdx, l, idx+n//2, minsum)
        
        for ur, urx, pmr in zip(urs, urxs, pmrs):
            u +=  [ul.tolist() + ur.tolist()]
            ux += [(ulx^urx).tolist() + urx.tolist()]
            pm += [pml + pmr]
    
    # Keep only the best candidates with lowest pm values:
    bestIndexes = np.argsort(pm)[:l]
    
    # The first output is a list of candidate u values
    # The second output is used internally and eventually will provide the list of
    # corresponding codewords for the candidates in the first output
    # The third output is a list of path metrics for each one of the candidates
    # All lists are sorted so that the candidate with lowest path metric comes first
    return np.int8(u)[bestIndexes], np.int8(ux)[bestIndexes], np.float32(pm)[bestIndexes]

# **********************************************************************************************************************
def testSc():
    # Tesing SC and SCL Algorithms
    n = 8
    k = 5
    d = int(np.log2(n))
    rate = k/n
    ...
    rs = np.int32([0, 1, 2, 4, 8, 3, 5, 9, 6, 10, 12, 7, 11, 13, 14, 15])
    rs = rs[rs<n]   # Assuming rs1024 is a numpy array from 5G standard
    frozenBits = rs[:n-k]
    msgBits = rs[n-k:n]
    print("\nReliability Sequence:", rs)
    print("Frozen Bits:         ", frozenBits)
    ...
    g = [1]
    for _ in range(d): g = np.kron([[1, 0], [1, 1]], g)

    codeWord = u.dot(g)%2
    print("Codeword:        ", codeWord)

    txBpsk = 1 - 2.0*codeWord

    ebNoDb = 5
    ebNo = 10.0**(ebNoDb/10)
    sigma = 1/np.sqrt(2*rate*ebNo)
    rx = np.random.normal(txBpsk, sigma)

    uHat, cwHat = scDecode(rx, frozenBits)
    print("\nUsing SC:        ", uHat[msgBits])

    l = 4 # Max number of candidates to keep
    uHats, cwHats, pms = sclDecode(rx, frozenBits, l, minsum=False)
    print("\nUsing SCL, Top %d candidates:"%(l))
    for i, (uHat,pm) in enumerate(zip(uHats,pms)):
        print("  ", uHat[msgBits], " Path Metric:",pm, ("(Best)" if i==0 else "") )
    print()
